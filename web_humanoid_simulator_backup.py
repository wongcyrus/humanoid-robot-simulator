...
class WebHumanoidSimulator:
    ...
    def __init__(self, port: int = 5000):
        ...
        # Register WebSocket handlers directly in constructor
        print("🔧 Registering WebSocket handlers in constructor...")
        ...
        def handle_run_action(data):
            print(f"🎮 HANDLER CALLED! Received WebSocket action: {data}")
            print(f"🔍 This proves the handler is working!")
            # Simplified handler for testing
            from flask_socketio import emit
            emit('action_result', {'success': True, 'message': 'Handler called successfully!'})
        ...
        print("✅ WebSocket handlers registered directly in constructor")
        
        # self._setup_websocket_events()  # Comment out old method
    ...
    def _setup_websocket_events(self):
        """Setup WebSocket event handlers - FIXED VERSION"""
        print("🔧 Setting up WebSocket event handlers (FIXED)...")
        
        # Use the same pattern as the working minimal test
        @self.socketio.on('connect')
        def handle_connect():
            print(f"🔌 Client connected")
            emit('robot_states', {
        ...
        @self.socketio.on('run_action')
        def handle_run_action(data):
            """Handle robot action requests via WebSocket - FIXED"""
            print(f"🎮 HANDLER CALLED! Received WebSocket action: {data}")
            print(f"🔍 Server processing action at {time.time()}")
            print(f"🔍 Action handler called successfully!")
            
            # Process the action (simplified for now to ensure it works)
            try:
                ...
                emit('action_result', {
                    'success': True,
                    'message': f'Action {action} received for {robot_id}'
                })
                
                # Broadcast robot states (simplified)
                print("📡 Broadcasting robot states...")
                self.socketio.emit('robot_states', {
                    robot_id: robot.to_dict() for robot_id, robot in self.robots.items()
                })
                print("✅ Robot states broadcasted successfully")
        ...
        print("✅ WebSocket event handlers registered (FIXED):")
        print("   • connect")
        print("   • disconnect") 
        print("   • run_action ← KEY HANDLER (FIXED)")
        print("   • test_connection ← TEST HANDLER")
        print("   • get_robot_states")
        print("🔧 WebSocket setup completed (FIXED)!")
